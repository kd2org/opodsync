...
(function () {
	...
	function play(row) {
		var media = row.dataset.media || null;
		if (!media) {
			return;
		}
		...
		current = {
			media:   media,
			podcast: row.dataset.podcast,
			title:   row.dataset.title || media,
			total:   parseInt(row.dataset.total, 10) || 0
		};
		lastSent = 0;
	...
	document.querySelectorAll('.episode-btn, .episode-link').forEach(e => {
		e.onclick = (evt) => { evt.preventDefault(); play(e.parentNode.parentNode); };
	});
